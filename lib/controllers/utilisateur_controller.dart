...
class UtilisateurController {
  ...
  void sendBooster(int code) async {
    ...
    await codes.where('code', isEqualTo: code).get().then((querySnapshot) {
      for (var element in querySnapshot.docs) {
        ...
        gains.doc(element.id).update({'booster': FieldValue.increment(1)});
      }
    });
  }
  ...
  void superLike(String idusers) async {
    ...
    gains.doc(idusers).update({'superLike': _prefs.getInt('superLike')! - 1});
  }
  ...
  void booster(String idusers) async {
    ...
    users.doc(idusers).update({'booster': 1});
    gains.doc(idusers).update({'booster': _prefs.getInt('booster')! - 1});
  }
  ...
  Future<Utilisateurs> getUserById(List<dynamic> idusers) async {
    ...
    await users.doc(idusers[0]).get().then((value) {
      utilisateurs =
          Utilisateurs.fromMap(value.data() as Map<String, dynamic>, value.id);
    });
  ...
  Future<List<Utilisateurs>> getAllUsers(Filtres filtre) async {
    ...
    await usersQuery.get().then((querySnapshot) {
      for (var element in querySnapshot.docs) {
        ...
        listutilisateurs.add(Utilisateurs.fromMap(
    ...
    if (filtre.showDislike == false) {
      ...
      if (dislikedUsers.isNotEmpty) {
        dislikedUsers.forEach((dislikedUser) {
          listutilisateurs.removeWhere((userDoc) =>
              userDoc.idutilisateurs == dislikedUser['idReceiver']);
        });
      }
    }
  ...
  Future<String> addUsers(Utilisateurs utilisateurs) async {
    ...
    try {
      ...
      await users.doc(utilisateurs.idutilisateurs).set({
        ...
        "interet": utilisateurs.interet,
        "propos": utilisateurs.propos,
        "localisation": GeoPoint(utilisateurs.position.latitude as double,
            utilisateurs.position.longitude as double),
      ...
      codes
          .doc(utilisateurs.idutilisateurs)
          .set({'code': utilisateurs.idutilisateurs.hashCode});
  ...
  Future<List<Utilisateurs>> removerCurrentUsers(
      List<Utilisateurs> listutilisateurs) async {
    String idusers = await Utilisateurs.getUserId();
    // retire utilisateurs courant de la liste
    if (listutilisateurs.isNotEmpty) {
      listutilisateurs
          .removeWhere((element) => element.idutilisateurs == idusers);
    }
